...
TEST_CASE("demosaic.amaze runs Halide CPU path for all Bayer patterns and updates metadata") {
    ...
    for (const auto& pattern : cpipe::tests::cfa_patterns()) {
        const auto run = cpipe::tests::run_demosaic_node(
            "com.cpipe.demosaic.amaze", &cpipe_link_builtin_demosaic_amaze, pattern, raw);
