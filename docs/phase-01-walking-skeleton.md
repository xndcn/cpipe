...
| PD-74 | P1 clang-tidy complexity scope            | P1 keeps targeted `readability-function-cognitive-complexity` suppressions on existing large parser / pipeline / Vulkan lifecycle functions rather than refactoring release-candidate control flow during CI closure. Mechanical clang-tidy findings are fixed in code; broader decomposition is left to the phase that next changes those functions. |
