...
| P2-PD-7    | Vulkan dispatch policy                  | Retires [P1-PD-71](phase-01-walking-skeleton.md#4-phase-decisions-pd-n). `cpipe-runtime` owns a `VulkanCommandBuffer` wrapper + per-frame `vkAllocateCommandBuffers` + timeline-semaphore wait. Halide AOT Vulkan kernels still produce SPIR-V via Halide's GPU backend, but cpipe binds the SPIR-V into a `VkShaderModule` + `VkComputePipeline` and records dispatches through `VulkanCommandBuffer::record_halide_kernel(...)`. Halide's internal Vulkan runtime is disabled at AOT codegen time (`HL_VULKAN_NO_HOST_RUNTIME`). |
...
| P2-PD-47   | Reverting [P1-PD-56] and [P1-PD-60]     | The "engine: Host" classification on `linearize.dng_lut`, `blacklevel.dng_levels`, `wb.dual_illuminant`, `colormatrix.dng_to_working` is reverted to `engine: Halide` (CPU AOT, with parameter buffers via P2-PD-5). Migrations land in T5, T7, and T8 respectively. |
...
| P2-PD-51   | demosaic.rcd Vulkan AOT                 | `demosaic.rcd` builds both `${Halide_HOST_TARGET}` (CPU) and `${Halide_HOST_TARGET}-vulkan-vk_float16` (carries P1-PD-58 FP16 target feature). The Vulkan path is the P2 proof of cpipe-owned Vulkan dispatch (P2-PD-7); Tracy GPU capture is filed locally during T2 verification. |
...
- **cpipe owns Vulkan dispatch.** [P1-PD-71](phase-01-walking-skeleton.md#4-phase-decisions-pd-n) accepted that Halide's internal Vulkan runtime drove the `demosaic.bilinear` GPU run. P2-PD-7 retires the slip: Halide AOT now emits the SPIR-V module + signature, and `cpipe::runtime::VulkanCommandBuffer` records the `vkCmdDispatch`, signals/awaits a timeline semaphore, and feeds Tracy's GPU context. `HL_VULKAN_NO_HOST_RUNTIME` strips Halide's runtime so cpipe controls the queue. This is the single biggest scheduler change of P2 and is the prerequisite for the OCIO-Vulkan compute path (P2-PD-6 / P2-PD-26).
...
**Acceptance criteria:**
- [ ] `demosaic.bilinear` Vulkan path executes via cpipe-owned `VkCommandBuffer` on the development RTX device; CPU↔GPU result matches.
- [ ] Tracy GPU capture from a P1 min-pipeline smoke shows `VulkanCommandBuffer::submit` GPU spans interleaved with CPU `Scheduler::dispatch_node` spans.
- [ ] On a no-Vulkan machine, the CPU AOT path runs unchanged.
- [ ] No leaked `VkPipeline` / `VkShaderModule` per RTX validation layer report.

**Verification:**
- [ ] `ctest -R test_vulkan_command_buffer` green.
- [ ] `ctest -R test_node_demosaic_bilinear_vulkan` green with cpipe-owned dispatch (the manual Tracy capture is filed under `docs/evidence/p2-t2-tracy.tracy.md`, not in CI).

**Dependencies:** T1.

**Files likely touched:** `include/cpipe/runtime/VulkanCommandBuffer.hpp`, `src/cpipe/runtime/VulkanCommandBuffer.cpp`, `src/cpipe/runtime/ComputeContext.cpp`, `cmake/HalideHelpers.cmake` (HL_VULKAN_NO_HOST_RUNTIME).
...
**Description.** Implement `com.cpipe.demosaic.rcd` from Sanz Rodríguez & Bayón 2014 — algorithm only, no GPLv3 source consulted. Halide generator targets both `${Halide_HOST_TARGET}` (CPU) and `${Halide_HOST_TARGET}-vulkan-vk_float16` (P2-PD-51). Four-CFA support via uniform pattern (P2-PD-48 carries the wider CFA gate). Author RT 5.10 reference golden EXR pair (P2-PD-34). Update `demosaic.bilinear` to use the CFA uniform path too (so all three demosaic nodes share the four-CFA semantics).

**Acceptance criteria:**
- [ ] `process()` produces FP16 RGBA on a 16×16 synthetic Bayer; PSNR ≥ 40 dB against RT 5.10 RCD reference.
- [ ] Vulkan AOT path runs via cpipe-owned dispatch (T2); CPU and Vulkan outputs match within 1 ULP.
- [ ] All four 2×2 Bayer patterns (RGGB/BGGR/GRBG/GBRG) produce correct demosaic.
- [ ] `out_metadata.clear_cfa()` and `add_applied_step("demosaic")` validated by manifest at freeze.

**Verification:**
- [ ] `ctest -R test_node_demosaic_rcd` green.
- [ ] `ctest -L golden` PSNR ≥ 40 dB for `demosaic.rcd`.

**Dependencies:** T5.

**Files likely touched:** `src/cpipe/nodes/demosaic/demosaic_rcd.{cpp,json,_generator.cpp}`, `src/cpipe/nodes/demosaic/demosaic_bilinear*.cpp` (CFA-pattern uniform), `tests/unit/test_node_demosaic_rcd.cpp`, `tests/golden/demosaic.rcd/{in,out}.exr`, `tools/golden/rt_render.sh` (extended).

**Estimated scope:** L (3 source + 1 test + 2 LFS goldens + RT script).
...
- **CFA-pattern uniform.** P2 widens `demosaic.bilinear` (P1) and ships `demosaic.rcd` / `demosaic.amaze` with a CFA-pattern uniform (`int4 cfa_pattern = {0,1,1,2}` for RGGB, `{2,1,1,0}` for BGGR, etc.). `submit_halide_with_params` passes the uniform via the param buffer; Halide generator branches at compile-time on the uniform's enum.
...
- **`HL_VULKAN_NO_HOST_RUNTIME`.** Setting this flag at Halide AOT codegen produces a SPIR-V kernel without Halide's embedded `vkCreateInstance` etc. cpipe's `VulkanCommandBuffer` does the `vkCmdDispatch`. Compatible with Halide v21's Vulkan backend; carries [P1-PD-58](phase-01-walking-skeleton.md#4-phase-decisions-pd-n) FP16 target feature for demosaic.bilinear / .rcd.
...
| 31 | `test_vulkan_command_buffer`                      | unit        | cpipe-owned dispatch on demosaic.bilinear; CPU↔GPU within 1 ULP |
...
| 37 | `test_node_demosaic_rcd`                          | unit        | bilinear demosaic on synthetic Bayer + four-CFA + RT 5.10 reference |
...
Halide v21 stays on FetchContent (P0-PD-31 / P1-PD-7); the only build-system change is `HL_VULKAN_NO_HOST_RUNTIME` (P2-PD-7) in `cmake/HalideHelpers.cmake`.
