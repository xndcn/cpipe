...
│       │   ├── lens/
│       │   │   ├── shading_gainmap*.{cpp,json,_dispatch.cpp}              # P2-PD-16 / P2-PD-64
...
**Description.** Implement `com.cpipe.demosaic.amaze` from Emil Martinec's public algorithm description — algorithm only, no GPLv3 source consulted. Halide CPU AOT (no Vulkan AOT in P2). Author the checked-in golden EXR pair; P2-PD-62 records why this T7 fixture is deterministic cpipe self-reference instead of the planned RT 5.10 reference.

**Acceptance criteria:**
- [x] PSNR ≥ 40 dB against the checked-in deterministic cpipe self-reference (P2-PD-62 carries the RT 5.10 refresh).
...
- [x] `ctest -L golden` PSNR ≥ 40 dB for `demosaic.amaze`.
...
**Description.** Implement `com.cpipe.demosaic.quad_bayer_remosaic` per P2-PD-15: 4×4 → 2×2 edge-aware gradient blend (Sony QBC whitepaper + MathWorks BSD reference). Halide CPU AOT. Widen the `DngReader` CFA gate to accept 4×4 Quad Bayer metadata. Author cpipe self-referenced golden (P2-PD-36 — no public reference). P2-PD-63 records why the planned Pixel 8 Pro native-QBC corpus fixture is replaced by synthetic 4×4 parser/node coverage in T8.

**Acceptance criteria:**
- [x] Synthetic 4×4 QBC DNG loads; `DngReader` reports 4×4 CFA; `Pipeline::load` accepts when remosaic is upstream of every demosaic.
- [x] `demosaic.quad_bayer_remosaic` produces 2×2 Bayer at same dims; RGGB pattern checked.
- [x] Golden PSNR ≥ 40 dB against cpipe self-reference.
...
- [x] Synthetic `Pipeline::load` shape with `quad_bayer_remosaic → linearize → blacklevel → demosaic.rcd` covered by `test_node_demosaic_quad_bayer_remosaic` (Pixel 8 Pro QBC corpus smoke deferred by P2-PD-63 / P2-R4).
...
- [x] Architecture §17 Q6 updated to "Resolved in P2-PD-18" (already landed before T9; confirmed unchanged).

**Verification:**
- [x] `ctest -R test_node_lens_shading_gainmap` green.
- [x] `ctest -R test_opcode_list_2_parser` green.
- [x] `ctest -L golden` PSNR ≥ 40 dB for `lens.shading_gainmap`.
...
**Description.** Implement `com.cpipe.denoise.guided_filter` (He 2010) — fast preview / chroma path. Implement `com.cpipe.denoise.wavelet_bayes_shrink` (Chang 2000 + scikit-image BSD3 reference) — chroma denoise. Both Halide CPU AOT. Author goldens (deterministic cpipe self-reference for both; P2-PD-68 carries the wavelet reference substitution).

**Acceptance criteria:**
- [x] `denoise.guided_filter` PSNR ≥ 40 dB on cpipe self-reference.
- [x] `denoise.wavelet_bayes_shrink` PSNR ≥ 40 dB on deterministic cpipe self-reference for `R16G16B16A16_SFLOAT` chroma (P2-PD-68 substitutes for the missing scikit-image workflow).
...
- [x] `ctest -L golden` PSNR ≥ 40 dB for both.
...
**Description.** Implement `com.cpipe.denoise.bm3d` (P2-PD-23) — two-stage Halide schedule (hard-threshold step1 + Wiener step2). NoiseProfile β/λ drives per-channel sigma; `sigma_override` param overrides. Reference: Dabov 2007 + Mäkinen 2020 BSD; deterministic cpipe self-reference golden in T14 (P2-PD-69 carries the IPOL fixture substitution). Implement `com.cpipe.sharpen.edge_aware_usm` (He 2010 / 2013 fast guided filter) — Halide CPU AOT. Deterministic cpipe self-reference golden in T14 (P2-PD-69 carries the RT fixture substitution).

**Acceptance criteria:**
- [x] BM3D on a 16×16 noisy synthetic patch reduces RMSE by ≥ 80 % vs ground truth; PSNR ≥ 40 dB against deterministic cpipe self-reference (P2-PD-69 substitutes for the missing IPOL workflow).
- [x] `sigma_override` param accepts a single float and overrides the metadata-derived sigma.
- [x] Sharpen produces visibly enhanced edges; PSNR ≥ 40 dB against deterministic cpipe self-reference (P2-PD-69 substitutes for the missing RT workflow).
...
- [x] Goldens PSNR ≥ 40 dB against their respective references.
- [x] `applied_steps += <node_id>`.

**Verification:**
- [x] `ctest -R "test_node_tone_(filmic_rgb|aces_filmic|reinhard)"` green.
- [x] `ctest -L golden` PSNR ≥ 40 dB on all three.
...
- [x] PSNR ≥ 40 dB against the checked-in deterministic Mertens reference (P2-PD-71 substitutes for the missing IPOL workflow).
...
- [x] Tetrahedral interpolation produces correct output on a 33³ LUT (verified by direct math comparison on a few colors).
- [x] PSNR ≥ 40 dB on golden.
...
| 39 | `test_node_demosaic_quad_bayer_remosaic`          | unit        | synthetic 4×4 QBC → 2×2 RGGB remosaic + pipeline-load shape |
