...
| PD-33 | P0 clang-tidy baseline                           | CI runs clang-tidy on `src/**/*.cpp` translation units only; headers are checked through first-party includes via `HeaderFilterRegex`. C++ module scanning is disabled because P0 uses no C++20 modules and configure-only compile commands otherwise reference generated `.modmap` files that do not exist until build time. The initial `.clang-tidy` disables mechanical style and low-signal C ABI/image-buffer checks while keeping the selected rule families active for remaining findings. |
...
| P0-R3 | `.clang-tidy` initial run produces dozens of warnings against third-party headers, Catch2 macros, C ABI glue, Halide registration macros, and image-buffer pointer code; PD-22 turns 20+ checks on simultaneously. | Low    | High       | `.clang-tidy` config restricts checks to first-party `include/` and `src/` paths through `HeaderFilterRegex`; CI runs clang-tidy on `src/**/*.cpp` translation units only, with PD-33 baseline suppressions for low-signal checks. |
