...
extern "C" int demosaic_rcd(halide_buffer_t* input, halide_buffer_t* cfa_pattern,
                            halide_buffer_t* output);

namespace {

int demosaic_rcd_param(halide_buffer_t* const* inputs, std::size_t n_inputs,
    ...
    cfa.dimensions = 1;
    cfa.dim = &dim;
    return demosaic_rcd(inputs[0], &cfa, outputs[0]);
}

}  // namespace

CPIPE_REGISTER_HALIDE_PARAM_FILTER("demosaic_rcd", &demosaic_rcd_param)
