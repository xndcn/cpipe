...
// NOLINTBEGIN(modernize-use-using,readability-named-parameter,bugprone-easily-swappable-parameters)
// NOLINTBEGIN(cppcoreguidelines-macro-usage)

#define CPIPE_ABI_MAJOR 0
#define CPIPE_ABI_MINOR 1
#define CPIPE_MAX_BUFFER_DIMS 8
...
typedef struct {
    int (*get_dims)(const cpipe_buffer_t*, uint8_t* ndim, uint32_t out_dims[CPIPE_MAX_BUFFER_DIMS]);
    int (*get_format)(const cpipe_buffer_t*, int* out);
    int (*get_kind)(const cpipe_buffer_t*, int* out);
    int (*get_stride)(const cpipe_buffer_t*, uint64_t out_stride[CPIPE_MAX_BUFFER_DIMS]);
    int (*get_color_role)(const cpipe_buffer_t*, const char** out);
...
typedef struct {
    ...
    int (*get_curve)(const cpipe_props_t*, const char* key, const float** x_values,
                     const float** y_values, size_t* n);
...
// NOLINTEND(modernize-use-using,readability-named-parameter,bugprone-easily-swappable-parameters)
